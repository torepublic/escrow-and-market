...
<?php endif; ?>
		<form id="afocus" class="ct-box info-box" method="get" accept-charset="utf-8" action="<?php echo forum_link($forum_url['search']) ?>">
			<div class="hidden">
...
<?php ($hook = get_hook('se_pre_keywords')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo "Name";//$lang_search['Keyword search'] ?></span></label><br />
						<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="keywords" size="10" maxlength="100" <?php echo ($advanced_search) ? '' : 'required' ?> /></span>
					</div>
				</div>
...
<?php ($hook = get_hook('se_pre_forum_fieldset')) ? eval($hook) : null; ?>
				<!--<fieldset class="ct-box info-box<?php echo ++$forum_page['item_count'] ?>">-->
...
<td >
<div class="main-head">
		<h2 class="hn"><span><?php echo $lang_search['Trade categories']; ?></span></h2>
</div>
	<table border="1">
		<tr class="<?php echo ($forum_page['item_count'] % 2 != 0) ? 'odd' : 'even' ?><?php if ($forum_page['item_count'] == 1) echo ' row1'; ?>">
		
<?php
	echo market_get_category_forums($forum_config['o_trade_category_id']);
?>
		</tr>
	</table>

<div class="main-head">
		<h2 class="hn"><span><?php echo $lang_search['Newest offers']; ?></span></h2>
</div>
	<table border="1">
				
<?php
	if (isset($_GET['category']) and is_numeric($_GET['category']))
		echo market_get_newest_offers($_GET['category'],$forum_config['o_trade_category_id'],$forum_config['o_problem_forum_id']);
	else
		echo market_get_newest_offers(0,$forum_config['o_problem_forum_id']); // display offers from all forums
?>
	</table>	
</td>
</tr>
