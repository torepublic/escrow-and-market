...
else if (isset($_GET['email']))
{
	...
	$query = array(
		'SELECT'	=> 'u.username, u.email, u.email_setting, u.pubkey, u.btcescrowaddress',
		'FROM'		=> 'users AS u',
	...
	if (isset($_POST['form_sent']))
	{
		...
		if (empty($errors))
		{
			...
			//$message =encrypt_message($message, $recipient_info['pubkey'], $recipient_info['email']); 
...
<?php ($hook = get_hook('mi_enctyption_checkbox')) ? eval($hook) : null; ?>
				<div class="txt-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="txt-box textarea required">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_misc['Encrypt message with recipient public key'] ?></span></label>
						<input type=CHECKBOX NAME="set_encryption">Encrypt message<p>
					</div>
				</div>
