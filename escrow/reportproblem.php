...
if ($fid == $forum_config['o_problem_forum_id'] || $fid == $forum_config['o_problem_archive_id'])
{
	$isproblemforum =1;
...
if ($isproblemforum && ( $forum_user['id']==$buyerid || $forum_user['id']==$sellerid))
	$problemtopicavalable = 1;
else
	$problemtopicavalable = 0;
...
$errors = array();
//echo count($_SESSION['escrow_details']);
...
if (isset($_POST['form_sent']) and isset($_SESSION['escrowinfo']))
{
	...
	escrow_notify_problem_occured($_SESSION['escrowinfo']);
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
		...
		else if ($fid)
		{
			...
			//echo " w escrow numer ".$_SESSION['escrow_details']['index'];
			//echo " ustawaim problemid = ".$new_tid;
		}
