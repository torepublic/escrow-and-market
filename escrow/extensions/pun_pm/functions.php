...
function pun_pm_send_message($body, $subject, $receiver_username, &$message_id)
{
	...
	$address = get_free_btcaddress($errors);
	...
	if ($role == 'seller')
		...
		$seller= $forum_user['username'];
		$buyerid = pun_pm_get_receiver_id($buyer, $errors);
		$sellerid = pun_pm_get_receiver_id($seller, $errors);
		}
	else
		{
		$seller = $receiver_username;
		$buyer = $forum_user['username'];
		$buyerid = pun_pm_get_receiver_id($buyer, $errors);
		$sellerid = pun_pm_get_receiver_id($seller, $errors);	
		}
	$buyermessage = sprintf($lang_pun_pm['Buyer message'], $buyer,$seller,
							$subject , $amount, $address);
	$buyersubject = sprintf($lang_pun_pm['Buyer subject'],$amount);
	$sellermessage= sprintf($lang_pun_pm['Seller message'], $seller, $buyer, $subject, $amount);
	$sellersubject = sprintf($lang_pun_pm['Seller subject'],$amount);
	...
	if ($message_id !== false)
	{
		...
		$query = array(
			'UPDATE'		=> 'pun_pm_messages',
			'SET'			=> 'status = \'sent\', receiver_id = '.$receiver_id.', lastedited_at = '.intval($now).', subject = \''.$forum_db->escape($subject).'\', body=\''.$forum_db->escape($body).'\'',
			'WHERE'			=> 'id = '.intval($message_id).' AND sender_id = '.intval($forum_user['id']).' AND (status = \'draft\' OR status = \'sent\')'
		);
	...
	else
	{
		...
		$query = array(
			'INSERT'		=> 'sender_id, receiver_id, status, lastedited_at, read_at, subject, body',
			'INTO'			=> 'pun_pm_messages',
			'VALUES'		=> '0'.', '.$buyerid.', \'sent\', '.$now.', 0, \''.$forum_db->escape($buyersubject).'\', \''.$forum_db->escape($buyermessage).'\''
		);
		...
		$query = array(
			'INSERT'		=> 'sender_id, receiver_id, status, lastedited_at, read_at, subject, body',
			'INTO'			=> 'pun_pm_messages',
			'VALUES'		=> '0'.', '.$sellerid.', \'sent\', '.$now.', 0, \''.$forum_db->escape($sellersubject).'\', \''.$forum_db->escape($sellermessage).'\''
		);
		...
		$query =array(
			...
			'INTO'		=>	'escrows',
			'VALUES'	=>	$now.', '.$buyerid.', '.$sellerid.', '.$amount.', \''.$subject.'\', 0, 0, \''.$address.'\''
		);
		$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);
	}
...
function pun_pm_encrypt_body($body ,$username)//, $pubkey, $email)
{
	$errors =array();
	$uid = pun_pm_get_receiver_id($username , $errors);
	$pubkey = pun_pm_get_pubkey($username);
	$email = pun_pm_get_email($username);
	print_r($body);
	global $gpgtempfileurl;
	//$temp_file_url = '../../cache/gpgtemp.php';
	$temp_file_url = $gpgtempfileurl;
	if ($pubkey!='' and trim($body)!='')
	{
		file_put_contents($temp_file_url,$pubkey);
		shell_exec("gpg --trust-model always --yes --import $temp_file_url");
		file_put_contents($temp_file_url,$body);
		$body = shell_exec("gpg --trust-model always --yes -q -e -a -r $email  -o /dev/stdout $temp_file_url");
	}
	
	return $body;
}

function pun_pm_send_form($username = '', $subject = '', $body = '', $message_id = false, $reply_form = false, $notice = false, $preview = false , $amount=0)
{
	...
	$minimum_amount = $forum_config['o_minimum_escrow_value'];
...
?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text required">
						<span class="fld-input"><input type="hidden" id="fld<?php echo $forum_page['fld_count'] ?>" name="pm_receiver" value="<?php echo $username; ?>" size="70" maxlength="255" required /></span>
					</div>
				</div>
...
?>
				...
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box select">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_pun_pm['Choose role'] ?></span> <small><?php echo $lang_pun_pm['Choose role notice'] ?></small></label><br />
						<span class="fld-input"><select id="fld<?php echo $forum_page['fld_count'] ?>" name="req_role">
...
<?php ($hook = get_hook('pun_pm_fn_send_form_pre_amount_input')) ? eval($hook) : null; ?>
				<div class="txt-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="txt-box textarea required">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_pun_pm['Amount'] ?></span> <small><?php echo sprintf($lang_pun_pm['Amount notice'], $minimum_amount); ?></small></label><br />
						<div class="txt-input"><span class="fld-input"><input name='req_amount' type="number" required value='<?php echo $amount?>'></input></span></div>
					</div>
				</div>
				
<?php ($hook = get_hook('pun_pm_fn_send_form_pre_pubkeytextarea_output')) ? eval($hook) : null; ?>
				<div class="txt-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="txt-box textarea required">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_pun_pm['Publickey'] ?></span></label>
						<div class="txt-input"><span class="fld-input"><textarea id="fld<?php echo $forum_page['fld_count'] ?>" name="pubkey" rows="4" cols="95" required><?php echo $pubkey; ?></textarea></span></div>
					</div>
				</div>
...
<?php ($hook = get_hook('pun_pm_fn_send_form_pre_textarea_output')) ? eval($hook) : null; ?>
				<div class="txt-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="txt-box textarea required">
						...
						<div class="txt-input"><span class="fld-input"><textarea id="fld<?php echo $forum_page['fld_count'] ?>" name="req_message" rows="5" cols="95" required><?php echo $body; ?></textarea></span></div>
					</div>
				</div>
