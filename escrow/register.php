...
if (isset($_GET['cancel']))
	redirect(forum_link('login.php'), $lang_profile['Reg cancel redirect']);

// User pressed agree but failed to tick checkbox
else if (isset($_GET['agree']) && !isset($_GET['req_agreement']))
	redirect('login.php', $lang_profile['Reg cancel redirect']);
...
else if (isset($_POST['form_sent']))
{
	...
	if (empty($errors))
	{
		...
		if (!is_valid_pubkey($pubkey) and trim($pubkey)!='')
		...
		$proper_invitation = get_proper_invitation($username2);
		...
		if (empty($errors))
		{
			...
			redirect('userlist.php', $lang_profile['Reg complete']);
		}
	}
}
...
<?php ($hook = get_hook('rg_register_pre_pubkey')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count']; if ($forum_config['o_regs_verify'] == '0') echo ' prepend-top'; ?>">
					<div class="sf-box text required">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_profile['GPG Public key (optional)'] ?></span> <small><?php echo $lang_profile['gpg help']; ?></small></label><br />
						<span class="fld-input"><textarea cols="100" rows="1" style="font-size:40%" id="fld<?php echo $forum_page['fld_count'] ?>" name="req_pubkey"><?php echo(isset($_POST['req_pubkey']) ? forum_htmlencode($_POST['req_pubkey']) : '') ?></textarea></span>
					</div>
				</div>
<?php ($hook = get_hook('rg_register_pre_btcaddress')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count']; if ($forum_config['o_regs_verify'] == '0') echo ' prepend-top'; ?>">
					<div class="sf-box text required">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo 'Bitcoin address' ?></span> <small><?php echo 'For escrow purpose. Best bitcoin client: <a href="http://electrum.org">Electrum</a> ' ?></small></label><br />
