	...
	($forum_user['g_moderator'] != '1' || $forum_user['g_mod_edit_users'] == '0' || $user['g_id'] == FORUM_ADMIN || $user['g_moderator'] == '1'))
{
	...
	if ($user['pubkey']!='')
		$forum_page['user_contact']['forum-pubkey'] = '<li><span>RSA public key: <textarea cols="70" rows ="10" readonly wrap="soft" syle="font-size: 50%; font-family: Courier">'.$user['pubkey'].'</textarea></span></li>';
	if ($user['btcaddress']!='')
		$forum_page['user_contact']['forum-btcaddress'] = '<li><span>BTC address: '.$user['btcaddress'].'</span></li>';
...
else
{
	...
	if ($section == 'about')
	{
		...
		if ($user['pubkey']!='')
			$forum_page['user_contact']['forum-pubkey'] = '<li><span>RSA public key: <textarea cols="70" rows ="10" readonly wrap="soft" syle="font-size:50%; font-family:Monospace">'.$user['pubkey'].'</textarea></span></li>';
		if ($user['btcaddress']!='')
			$forum_page['user_contact']['forum-btcaddress'] = '<li><span>BTC address: '.$user['btcaddress'].'</span></li>';
...
<?php ($hook = get_hook('pf_change_details_identity')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						...
						<span class="fld-input"><textarea id="fld<?php echo $forum_page['fld_count'] ?>" cols="60" rows="1" name="form[pubkey]"  style="font-size:50%" >"<?php echo(isset($form['pubkey']) ? validate_pubkey(forum_htmlencode($form['pubkey'])) : validate_pubkey(forum_htmlencode($user['pubkey']))) ?>"</textarea> </span>
					</div>
				</div>
