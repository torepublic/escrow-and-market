...
$forum_index = get_forum_id_by_problemid($id);
if ($forum_index == $forum_config['o_problem_forum_id'] || $forum_index == $forum_config['o_problem_archive_id'])
{
	$isproblemforum =1;
...
$problemtopicavalable = $isproblemforum && ( $forum_user['id']==$buyerid || $forum_user['id']==$sellerid);



if (!$problemtopicavalable){
...
$query = array(
	...
	'WHERE'		=> 't.id='.$id.' AND t.moved_to IS NULL'
);	
}
...
if (!$cur_topic )//and !$problemtopicavalable)
{
	message($lang_common['Bad request']);
}
...
if ($cur_topic['closed'] == '0' || $forum_page['is_admmod'])
	$forum_user['may_post'] = (($cur_topic['post_replies'] == '' && $forum_user['g_post_replies'] == '1')
	 || $cur_topic['post_replies'] == '1' || $forum_page['is_admmod']
	 || $problemtopicavalable) ? true : false;
else
